# nf-core/vcreport: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/vcreport/usage](https://nf-co.re/vcreport/usage)
...
```console
nextflow run nf-core/vcreport --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/vcreport
```
...
First, go to the [nf-core/vcreport releases page](https://github.com/nf-core/vcreport/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
